...
跨境班次必须按具体站点查询，一次只能查一个出发站点→目的站点的组合。如果目的地城市有多个站点，每个站点需要单独调用一次接口，**会成倍消耗 token 和时间**。

因此，在查询前必须先确认站点：

- 如果用户已明确指定站点（如"香港机场到皇岗口岸"）→ 直接查询
- 如果用户只说了城市（如"香港到深圳"）→ 执行下方【站点确认流程】
...
WHERE company_id = 2
  AND del_flag = 0
  AND (city LIKE '%{城市关键词}%' OR name LIKE '%{城市关键词}%')
ORDER BY name
```

根据查询结果处理：

**只有 1 个站点** → 直接使用，无需询问用户。
...
2. {站点名称B}
3. {站点名称C}
（如需查所有站点的班次，请回复"全部"，注意将逐个查询，耗时较长）
```

**有 4 个及以上站点** → 必须询问用户，不得自行全查：
```
{城市}有 {N} 个跨境站点，建议指定站点查询以节省时间：
1. {站点名称A}
2. {站点名称B}
...
请问您要查哪个站点？如需查所有站点，请明确告知，但注意需逐个查询，会消耗较多时间和资源。
```
...
确认好站点名称后，查询对应的 `open_station_id`：

```sql
SELECT open_station_id, name
FROM lulu_station
WHERE company_id = 2
  AND del_flag = 0
  AND name = '{站点名称}'
...
每个出发站点→目的站点组合调用一次 `ticket_api` tool：
...
如需查询多个站点组合，逐个调用后汇总所有结果再统一展示。

### 第五步：处理结果

**无班次：** 告知"该日期 {出发站点}→{目的站点} 暂无可用跨境班次"

**有班次（单站点查询）：** 根据用户问题筛选展示：
...
**有班次（多站点汇总）：** 合并所有站点的结果，按用户需求筛选（如最晚班次取所有结果中 departureTime 最大的一条），展示时注明所属站点。
